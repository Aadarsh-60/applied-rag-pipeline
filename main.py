  ...
  python main.py --data-dir /path/to/docs --index-path /path/to/index
    ...
    parser.add_argument(
        ...
        help="LLM to use for answer generation. "
             "Options: groq-llama3, gpt-4, ollama/llama3, ollama/mistral. "
             "Default: groq-llama3",
    )
...
def init_pipeline(args):
    """
    Initialize and return the full RAG pipeline.

    This function orchestrates all 6 setup steps, printing clear separators between
    ...
    if not args.model.startswith("ollama/") and not os.getenv("GROQ_API_KEY"):
        print(
            "\n⚠️  WARNING: GROQ_API_KEY is not set in your environment.\n"
            "   Either:\n"
...
def interactive_loop(qa_chain, args):
    """
    Run the interactive Q&A loop or answer a single question.
    """
    # -------------------------------------------------------------------------
...
if __name__ == "__main__":
    args = parse_args()
    qa_chain = init_pipeline(args)
    if qa_chain:
        interactive_loop(qa_chain, args)
